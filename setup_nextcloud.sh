...
PASSWORD=${1?"Usage: $0 <password> <nextcloud root> <primary storage> [domain name]"}
NEXTCLOUD_ROOT=${2?"Usage: $0 <password> <nextcloud root> <primary storage> [domain name]"}
PRIMARY_STORAGE=${3?"Usage: $0 <password> <nextcloud root> <primary storage> [domain name]"}
DOMAIN_NAME=$4
...
sudo sed -i "/);/i \ \ 'overwrite.cli.url' => 'https://$DOMAIN_NAME/nextcloud'," $NEXTCLOUD_DIR/config/config.php
